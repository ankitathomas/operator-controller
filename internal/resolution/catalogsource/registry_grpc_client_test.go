...
type mockServer struct {
	packages map[string]*api2.Package
	bundles  []*api2.Bundle
	api2.UnimplementedRegistryServer
}

func (s *mockServer) ListBundles(_ *api2.ListBundlesRequest, stream api2.Registry_ListBundlesServer) error {
...
func (s *mockServer) GetPackage(_ context.Context, req *api2.GetPackageRequest) (*api2.Package, error) {
	if req != nil {
		if p, ok := s.packages[req.Name]; ok {
			return p, nil
		}
	}
	return &api2.Package{}, nil
}

var _ = Describe("Registry GRPC Client", func() {
	testPackages := map[string]*api2.Package{
		"prometheus": {
			Name: "prometheus",
			Channels: []*api2.Channel{{
				Name:    "beta",
	...
	testBundles := []*api2.Bundle{{
		...
		ProvidedApis: []*api2.GroupVersionKind{{
	...
	BeforeEach(func() {
		...
		api2.RegisterRegistryServer(grpcServer, &mockServer{packages: testPackages, bundles: testBundles})
